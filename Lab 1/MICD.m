...
figure;
hold on;
imagesc(x_range,y_range,decision_micd);
set(gca,'ydir','normal');
% colormap for the classes:
cmap = [1 0.8 0.8; 0.95 1 0.95; 0.9 0.9 1;];
colormap(cmap);
contour(x,y,decision_micd,'k');
...
xlabel('x_1');
ylabel('x_2');
title("MICD Decision Boundary Case 1");
hold off

% error analysis
distA = GED(cluster_A',mu_A,mu_B,sig_A,sig_B);
distB = GED(cluster_B',mu_A,mu_B,sig_A,sig_B);
trueA = ones(N_a,1);
...
fprintf('MICD Case 1 Experimental Error Rate: %f percent\n',prediction_error)

% confusion matrix
C = confusionmat([trueA ;trueB],[predA ;predB2],"Order",[1,0]);
figure
confusionchart(C,["A","B"])
title("Confusion Matrix MICD - Case 1")
...
xlabel('x_1');
ylabel('x_2');
title("MICD Decision Boundary Case 2");
hold off

% error analysis
% Cluster C is class ONE
distcd = GED(cluster_C',mu_C,mu_D,sig_C,sig_D);
distde = GED(cluster_C',mu_D,mu_E,sig_D,sig_E);
distec = GED(cluster_C',mu_E,mu_C,sig_E,sig_C);
trueC = ones(N_c,1);
...
distcd = GED(cluster_D',mu_C,mu_D,sig_C,sig_D);
distde = GED(cluster_D',mu_D,mu_E,sig_D,sig_E);
distec = GED(cluster_D',mu_E,mu_C,sig_E,sig_C);
trueD = 2*ones(N_d,1);
...
distcd = GED(cluster_E',mu_C,mu_D,sig_C,sig_D);
distde = GED(cluster_E',mu_D,mu_E,sig_D,sig_E);
distec = GED(cluster_E',mu_E,mu_C,sig_E,sig_C);
trueE = 3*ones(N_e,1);
...
fprintf('MICD Case 2 Experimental Error Rate: %f percent\n',prediction_error)

% confusion matrix
C = confusionmat([trueC ;trueD ; trueE],[predC2 ; predD2; predE2],'Order',[1,2,3]);
figure
confusionchart(C,["C","D","E"])
title("Confusion Matrix MICD - Case 2")